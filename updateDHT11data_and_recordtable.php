<?php
  ...
  if (!empty($_POST)) {
    ...
    $sql = "UPDATE dht11 SET temperature = ?, humidity = ?, status_read_sensor_dht11 = ?, datetime = ? WHERE id = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($temperature,$humidity,$status_read_sensor_dht11,$dt,$id));
    Database::disconnect();
    ...
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		...
		$sql = "INSERT INTO dht11 (temperature,humidity,status_read_sensor_dht11) values(?, ?, ?)";
		$q = $pdo->prepare($sql);
		$q->execute(array($temperature,$humidity,$status_read_sensor_dht11));
    //::::::::
